...
import {Search, MousePointerClick, PhoneCall, CalendarCheck, TrendingUp} from 'lucide-react';
import {cn} from '@/lib/cn';

const ICONS = [Search, MousePointerClick, PhoneCall, CalendarCheck, TrendingUp];

type Stage = {title: string; body: string};

/**
 * Interactive 5-stage patient flow. Each step is a clickable card; the detail
 * panel below expands the active step. Horizontal + connected on desktop,
 * vertical stack on mobile. Keyboard accessible (real buttons).
 */
export function PatientFlow({stages, intro}: {stages: Stage[]; intro: string}) {
            ...
            <Fragment key={s.title}>
              <button
                ...
                className={cn(
                  ...
                  isActive
                    ? 'border-blue/40 bg-blue/5 shadow-card'
                    : 'border-border bg-bg hover:-translate-y-0.5 hover:scale-[1.02] hover:border-blue/25 hover:shadow-card'
                )}
              >
                <span
                  className={cn(
