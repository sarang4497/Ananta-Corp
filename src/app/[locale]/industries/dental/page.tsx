...
import {LandingPreview} from '@/components/dental/LandingPreview';
import {LeakyFunnel} from '@/components/dental/LeakyFunnel';
import {PatientFlow} from '@/components/dental/PatientFlow';
import {ImpactCharts} from '@/components/dental/ImpactCharts';
...
// ── TESTIMONIAL VIDEO ────────────────────────────────────────────────────────
// Paste your YouTube video ID or full link here. Empty = "coming soon" placeholder.
// https://youtu.be/vSHgV3tcfgA
const DENTAL_TESTIMONIAL_YOUTUBE = 'vSHgV3tcfgA';
...
const SERVICE_ICONS: LucideIcon[] = [
  Megaphone, Sparkles, PhoneCall, Smile, CalendarCheck, ClipboardList, Search, Star, Globe
];
const DIFF_ICONS: LucideIcon[] = [Target, ClipboardList, TrendingUp, MapPin];
...
const DIFF_ACCENTS: Accent[] = ['blue', 'indigo', 'green', 'orange'];
const SERVICE_ACCENTS: Accent[] = [
  'blue', 'indigo', 'green', 'orange', 'red', 'blue', 'indigo', 'green', 'orange'
];
...
export async function generateMetadata({params}: Params): Promise<Metadata> {
  ...
  const t = await getTranslations('pages.industries.dental.meta');
  return {
    title: t('title'),
    description: t('description'),
    alternates: buildAlternates({pathname: '/industries/[slug]', params: {slug: 'dental'}}, locale)
  };
}

export default async function DentalPage({params}: Params) {
  const {locale} = await params;
  setRequestLocale(locale);
  const t = await getTranslations('pages.industries.dental');
  ...
  return (
    <>
      ...
      {/* 2 · Stats (stacked, left) + testimonial video (right). Stacks on mobile. */}
      <section className="shell pb-10 pt-2 sm:pb-12">
        <div className="grid items-center gap-8 lg:grid-cols-2 lg:gap-12">
          <Reveal
            stagger={0.08}
            className="mx-auto grid w-full max-w-sm grid-cols-2 gap-x-6 gap-y-8 lg:max-w-md lg:gap-y-10"
          >
            {stats.map((s, i) => (
          ...
          <Reveal className="flex flex-col gap-3">
            <span className="text-center text-[11px] font-bold uppercase tracking-[0.18em] text-indigo">
              {t('testimonial.heading')}
            </span>
            <DentalTestimonialVideo
              youtubeId={ytId(DENTAL_TESTIMONIAL_YOUTUBE)}
      ...
      {/* 3 · Sample landing page preview — large, full content width. */}
      <section className="shell pb-12 sm:pb-14">
        <Reveal className="flex flex-col items-center gap-6">
          <SectionHeading title={t('landingPreview.heading')} />
          <LandingPreview heading={t('landingPreview.heading')} cta={t('landingPreview.cta')} />
        </Reveal>
      </section>

      {/* 3 · The Market Reality + leaky funnel */}
      ...
      <section className="border-y border-border bg-bg-soft/40">
        <div className="shell py-12 sm:py-14">
          ...
          <Reveal className="mt-8 sm:mt-10">
            <PatientFlow stages={flowStages} intro={t('flow.intro')} />
          </Reveal>
        </div>
      </section>
      ...
      {/* 7 · Dental services grid (anchor IDs match the Industries nav) */}
      <section className="border-y border-border bg-bg-soft/40">
        <div className="shell py-12 sm:py-14">
          <Reveal>
