...
const SERVICE_ICONS: LucideIcon[] = [
  Target, Headphones, FileText, Database, UserPlus, Search, Award, Globe
];
const DIFF_ICONS: LucideIcon[] = [Target, FileText, Database, Sparkles];
// Names (strings) — IndustryFlow is a client component and resolves them.
const FLOW_ICONS = ['Target', 'PhoneCall', 'FileText', 'Handshake', 'TrendingUp'];
...
const DIFF_ACCENTS: Accent[] = ['orange', 'blue', 'indigo', 'green'];
const SERVICE_ACCENTS: Accent[] = [
  'orange', 'blue', 'indigo', 'green', 'red', 'orange', 'blue', 'indigo'
];

// Illustrative B2B lead funnel (numbers baked; prose comes from messages).
const FUNNEL_STAGES: FunnelStage[] = [
  {label: 'Ad spend', value: '$2,000', width: 100, color: '#1877F2'},
  {label: 'Leads', value: '40', width: 80, color: '#3A66E0'},
  {label: 'Qualified', value: '15', width: 60, color: '#4F46E5'},
  {label: 'Meetings', value: '6', width: 42, color: '#6D5BD0'},
  {label: 'Deals', sub: 'signed', value: '2', width: 26, color: '#B06A9E'}
];
...
export async function generateMetadata({params}: Params): Promise<Metadata> {
  ...
  const t = await getTranslations('pages.industries.engineering.meta');
  return {
    title: t('title'),
    description: t('description'),
    alternates: buildAlternates(
      {pathname: '/industries/[slug]', params: {slug: 'engineering-b2b'}},
      locale
    )
  };
}

export default async function EngineeringPage({params}: Params) {
  const {locale} = await params;
  setRequestLocale(locale);
  const t = await getTranslations('pages.industries.engineering');

  const stats = t.raw('stats') as StatItem[];
  const diagram = t.raw('diagram') as {
    caption: string;
    payoffLead: string;
    payoffLeadValue: string;
    payoffIn: string;
    payoffOut: string;
  ...
  const payoff: FunnelPayoff = {
    kind: 'multiplier',
    lead: diagram.payoffLead,
    leadValue: diagram.payoffLeadValue,
    inLabel: diagram.payoffIn,
    outLabel: diagram.payoffOut,
    multiplier: diagram.payoffMultiplier
  };
  ...
  return (
    <>
      ...
      {/* 3 · The Market Reality + B2B lead funnel */}
