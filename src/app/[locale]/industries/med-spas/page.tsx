...
import {ValueCompare} from '@/components/industries/ValueCompare';
...
const SERVICE_ICONS: LucideIcon[] = [
  PhoneCall, CalendarCheck, ClipboardCheck, MapPin, Star, MessageCircle, Globe
];
const DIFF_ICONS: LucideIcon[] = [PhoneCall, ClipboardCheck, Star, Sparkles];
// Names (strings) — IndustryFlow is a client component and resolves them.
const FLOW_ICONS = ['PhoneCall', 'CalendarCheck', 'ClipboardCheck', 'Star', 'MessageCircle'];
...
const DIFF_ACCENTS: Accent[] = ['green', 'indigo', 'blue', 'orange'];
const SERVICE_ACCENTS: Accent[] = ['green', 'blue', 'indigo', 'orange', 'red', 'green', 'blue'];
...
export async function generateMetadata({params}: Params): Promise<Metadata> {
  ...
  const t = await getTranslations('pages.industries.medSpas.meta');
  return {
    title: t('title'),
    description: t('description'),
    alternates: buildAlternates(
      {pathname: '/industries/[slug]', params: {slug: 'med-spas'}},
      locale
    )
  };
}

export default async function MedSpasPage({params}: Params) {
  const {locale} = await params;
  setRequestLocale(locale);
  const t = await getTranslations('pages.industries.medSpas');

  const stats = t.raw('stats') as StatItem[];
  const diagram = t.raw('diagram') as DiagramLabels;
  ...
  const chart1: ChartSpec = {
    ...
    unit: 'eur',
    data: [
      {label: 'Single visit', v: 150},
      {label: 'Retained', v: 1500}
    ]
  };
  ...
  return (
    <>
      ...
      {/* 3 · The Market Reality + client lifetime value diagram */}
        ...
        <div className="shell py-12 sm:py-14">
          ...
          <Reveal className="mx-auto mt-10 w-full max-w-5xl sm:mt-12">
            <ValueCompare
              ...
              oldLabel={diagram.oldLabel}
              oldValue={150}
              oldItems={diagram.oldItems}
              oldDesc={diagram.oldDesc}
              newLabel={diagram.newLabel}
              newValue={1500}
              newItems={diagram.newItems}
              newDesc={diagram.newDesc}
              currency="€"
              uplift={diagram.uplift}
              note={diagram.note}
            />
          </Reveal>
        </div>
